  ...
  return (
    <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-[100] p-4 font-serif" onClick={onClose}>
      <div className="bg-[#1a1814] border-2 border-[#a67c52] rounded-md p-6 max-w-sm w-full shadow-2xl relative flex flex-col items-center gap-6" onClick={e => e.stopPropagation()}>
        ...
        <div className="w-full max-w-[280px] aspect-[2/3] bg-[#1a1814] border-2 border-[#a67c52] rounded-md overflow-hidden relative shadow-lg">
          ...
          {card.imageUrl ? (
            <img src={card.imageUrl} referrerPolicy="no-referrer" alt={card.name} className="w-full h-full object-contain" />
          ) : (
        ...
        <div className="w-full bg-[#0f0e0c] border border-[#3d3326] p-4 rounded text-center space-y-3">
          ...
          {card.effects && card.effects.length > 0 && (
            <div className="pt-2 border-t border-[#3d3326] flex flex-col gap-2">
              {card.effects.map((eff, i) => (
                <div key={i} className="flex flex-col gap-1 items-center">
                  <span className="bg-[#3d3326] text-xs font-bold text-[#e2b17a] uppercase px-2 py-1 rounded border border-[#a67c52]/50 w-fit">
                    {eff}
                  </span>
                  <p className="text-[10px] text-[#d4c3a1]/80 italic">
